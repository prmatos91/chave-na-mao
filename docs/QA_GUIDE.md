...
| Integração com Postgres real | `backend/src/test/java/.../CrmCarrosPostgresIntegrationTest.java` | Testcontainers | Migrations e mapeamento JPA contra um PostgreSQL real (não H2). Pula a si mesmo de forma limpa se o Docker não estiver acessível ao Testcontainers no ambiente — não é uma falha, ver seção 5.2 |
| Unitário (componentes/serviços) | `frontend/src/app/**/*.spec.ts` | Vitest (via `@angular/build:unit-test`) + Angular TestBed | Validação de formulários reativos, montagem de requisições HTTP, renderização condicional de dados, e serviços/diretivas de UI isolados (`ThemeService`, `paginatorIntlPtBrFactory`, `SelectOnFocusDirective`) |
| Build/compilação | `mvnw clean package` / `ng build` | Maven / esbuild | Ausência de erros de tipo/compilação em todo o projeto |
| End-to-end automatizado | `frontend/e2e/*.spec.ts` | Playwright, contra a stack real via Docker Compose | Fluxos de ouro completos pela UI real: CRUD de veículo e cliente, validações, o fluxo de ponta a ponta da regra de negócio (oportunidade Vendido → veículo Vendido), identidade visual/título, tradução do paginador, toggle de tema claro/escuro, autocomplete marca→modelo em cascata, o fix de UX do campo Preço (zero padrão substituído ao digitar), ordenação de colunas, navegação do dashboard para listagens filtradas, e as entidades relacionadas nas telas de detalhe |
