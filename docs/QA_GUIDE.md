...
- A suíte rápida do backend (`mvnw test` no dia a dia) roda contra **H2 em modo PostgreSQL**. A suíte que valida contra **PostgreSQL real** (`CrmCarrosPostgresIntegrationTest`, via Testcontainers) existe e é real, mas em pelo menos um ambiente de desenvolvimento observado (Docker Desktop muito recente) ela não conseguiu detectar o Docker disponível por uma incompatibilidade de baixo nível entre o Testcontainers e essa versão específica do Docker Desktop — o teste pula a si mesmo de forma limpa nesse caso (não falha o build), e deve rodar normalmente em CI/ambientes com Docker Engine padrão. Ver o relato completo, real, em [TEST_EXECUTION_LOG.md](TEST_EXECUTION_LOG.md) seção 5.2 antes de assumir que é um problema no código.
